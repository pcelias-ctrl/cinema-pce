...
public_layout($action === 'catalog' ? 'Ingressos' : 'Minha conta', $cinema, $customer, $action, $message, $error, function () use ($action,$cinema,$customer,$dates,$selectedDate,$parsedDate,$movies,$seatContext,$orderContext,$paymentContext,$accountOrders,$products,$portalSettings) {
    ...
    <?php } elseif ($action === 'checkout') { if(!$orderContext){?><section class="empty-state"><strong>Reserva indisponível</strong><p>Escolha as poltronas novamente.</p><a class="portal-button primary" href="/">Ver sessões</a></section><?php return;}?>
        ...
        <?php $gateway=$portalSettings['payment_gateway']==='infinitepay'?'infinitepay':'pagarme';$gatewayReady=$gateway==='infinitepay'?!empty($portalSettings['infinitepay_handle']):(!empty($portalSettings['pagarme_public_key'])&&!empty($portalSettings['pagarme_secret_encrypted']));?>
        <form method="post" action="/?action=pay" class="public-checkout" id="public-checkout-form" data-gateway="<?=e($gateway)?>" data-ticket-total="<?=e($orderContext['tickets_total'])?>" data-pagarme-key="<?=e($portalSettings['pagarme_public_key'])?>"><input type="hidden" name="csrf_token" value="<?=e(csrf_token())?>"><input type="hidden" name="order_code" value="<?=e($orderContext['order_code'])?>"><input type="hidden" name="card_token" id="card-token"><section><h2>Ingressos</h2><div class="checkout-lines"><?php foreach($orderContext['holds'] as $hold):?><div><span>Poltrona <?=e($hold['seat_code'])?> · <?=e(ucfirst($hold['ticket_type']))?></span><strong>R$ <?=e(number_format((float)$hold['unit_price'],2,',','.'))?></strong></div><?php endforeach;?></div><h2>Adicionar produtos</h2><div class="public-product-grid"><?php foreach($products as $product):?><article><?php if($product['has_image']):?><img src="/?action=product_image&id=<?=(int)$product['id']?>" alt=""><?php endif;?><div><strong><?=e($product['name'])?></strong><span><?=e($product['category_name'])?></span><b>R$ <?=e(number_format((float)$product['price'],2,',','.'))?></b></div><input name="product_qty[<?=(int)$product['id']?>]" type="number" min="0" max="10" value="0" data-product-price="<?=e($product['price'])?>" aria-label="Quantidade de <?=e($product['name'])?>"></article><?php endforeach;?></div></section><aside class="checkout-summary"><h2>Resumo</h2><div><span>Ingressos</span><strong>R$ <?=e(number_format((float)$orderContext['tickets_total'],2,',','.'))?></strong></div><div><span>Produtos</span><strong id="checkout-products-total">R$ 0,00</strong></div><div class="grand"><span>Total</span><strong id="checkout-grand-total">R$ <?=e(number_format((float)$orderContext['total_amount'],2,',','.'))?></strong></div><?php if($gateway==='infinitepay'):?><input type="hidden" name="payment_method" id="public-payment-method" value="pix"><div class="gateway-choice"><strong>InfinitePay</strong><span>Escolha Pix ou cartão na próxima etapa.</span></div><?php else:?><label>Pagamento<select name="payment_method" id="public-payment-method"><option value="pix">Pix</option><option value="cartao">Cartão</option></select></label><div class="card-fields" id="public-card-fields" hidden><label>Número do cartão<input id="card-number" inputmode="numeric" autocomplete="cc-number"></label><label>Nome impresso<input id="card-holder" autocomplete="cc-name"></label><div><label>Validade<input id="card-expiry" placeholder="MM/AA" inputmode="numeric" autocomplete="cc-exp"></label><label>CVV<input id="card-cvv" inputmode="numeric" maxlength="4" autocomplete="cc-csc"></label></div></div><?php endif;?><button class="portal-button primary" id="public-pay-button" <?=(!(int)$portalSettings['sales_enabled']||!$gatewayReady)?'disabled':''?>>Continuar para pagamento</button><?php if(!(int)$portalSettings['sales_enabled']):?><small>Pagamento online temporariamente indisponível.</small><?php elseif(!$gatewayReady):?><small>Gateway aguardando configuração.</small><?php else:?><small>Pagamento processado com segurança pelo <?=$gateway==='infinitepay'?'InfinitePay':'Pagar.me'?>.</small><?php endif;?></aside></form>
