...
$publicSalesEnabled = (int) ($portalSettings['sales_enabled'] ?? 0) === 1;
...
if ($action === 'seats' && !$publicSalesEnabled) {
    $error = 'A venda de ingressos pela internet está temporariamente desabilitada.';
    $action = 'catalog';
}
...
if ($action === 'hold' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!$publicSalesEnabled) {
        $_SESSION['portal_error'] = 'A venda de ingressos pela internet está temporariamente desabilitada.';
        header('Location: /');
        exit;
    }
    if (!$customer) { header('Location: /?action=access'); exit; }
...
if ($action === 'pay' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try{
        if(!$publicSalesEnabled)throw new RuntimeException('A venda de ingressos pela internet está temporariamente desabilitada.');
...
public_layout($action === 'catalog' ? 'Ingressos' : 'Minha conta', $cinema, $customer, $action, $message, $error, function () use ($action,$cinema,$customer,$dates,$selectedDate,$parsedDate,$movies,$presaleMovies,$comingSoonMovies,$seatContext,$orderContext,$paymentContext,$accountOrders,$products,$portalSettings,$publicSalesEnabled) {
    ...
    <?php } else { ?>
        <?php $onlineSalesEnabled = !empty($portalSettings['sales_enabled']); ?>
        <section class="catalog-heading"><div><span class="eyebrow">Programação</span><h1>Escolha sua sessão</h1><p><?=$onlineSalesEnabled?'Selecione o filme, horário e poltronas.':'Consulte os filmes e horários. A venda pela internet está temporariamente pausada.'?></p></div><?php if($customer):?><span class="customer-chip">Olá, <?=e(explode(' ',$customer['name'])[0])?></span><?php endif;?></section>
        ...
        <?php if($presaleMovies):?>
            <section class="presale-strip"><div class="section-strip"><h2>Pré-venda</h2></div><div class="presale-grid"><?php foreach($presaleMovies as $movie):$info=$movie['info'];?><article class="presale-card"><?php if(!empty($info['has_promo_banner'])||$info['has_cover']):?><button type="button" class="presale-banner-button" data-open-modal="movie-modal-<?=(int)$info['movie_id']?>" aria-label="Ver detalhes de <?=e($info['title'])?>"><img src="<?=$info['has_promo_banner']?'/?action=movie_banner&id='.(int)$info['movie_id']:'/?action=cover&id='.(int)$info['movie_id']?>" alt="Banner de <?=e($info['title'])?>"></button><?php endif;?><div><button type="button" class="movie-open-button" data-open-modal="movie-modal-<?=(int)$info['movie_id']?>"><span><?=e($info['title'])?></span><?=public_age_badge($info['age_rating'])?></button><p><?=e($info['genre'])?> · <?=(int)$info['duration_minutes']?> min</p><div class="session-groups"><?php foreach($movie['sessions'] as $session):$available=max(0,(int)$session['capacity']-(int)$session['sold']-(int)$session['held']);$released=!empty($session['buy_allowed']);$canBuy=$onlineSalesEnabled&&$released&&$available>0;?><a class="session-button <?=$canBuy?'':'sold-out'?> <?=!$onlineSalesEnabled?'sales-disabled':''?>" <?=$canBuy?'href="/?action=seats&amp;showtime_id='.(int)$session['id'].'"':'aria-disabled="true"'?>><strong><?=e(date('H:i',strtotime($session['starts_at'])))?></strong><span><?=e(ucfirst($session['audio_type']))?></span><span class="format-badges"><?=public_session_format_badges($session)?></span><small><?=!$onlineSalesEnabled?'Venda online pausada':($released?($available>0?$available.' lugares':'Esgotada'):'Libera em '.e(date('d/m H:i',strtotime((string)$session['presale_starts_at']))))?></small><?php if($released&&$available>0):?><em>Comprar</em><?php endif;?></a><?php endforeach;?></div></div></article><?=public_movie_modal($info,$movie['sessions'])?><?php endforeach;?></div></section>
        <?php endif;?>
        ...
        <div class="movie-program"><?php foreach($movies as $movie):$info=$movie['info'];?><article class="program-card"><?php if($info['has_cover']):?><button type="button" class="poster poster-button" data-open-modal="movie-modal-<?=(int)$info['movie_id']?>" aria-label="Ver detalhes de <?=e($info['title'])?>"><img src="/?action=cover&id=<?=(int)$info['movie_id']?>" alt="Capa de <?=e($info['title'])?>"></button><?php else:?><button type="button" class="poster poster-button" data-open-modal="movie-modal-<?=(int)$info['movie_id']?>"><div class="poster-placeholder">SEM CAPA</div></button><?php endif;?><div class="program-info"><div><button type="button" class="movie-open-button" data-open-modal="movie-modal-<?=(int)$info['movie_id']?>"><span><?=e($info['title'])?></span><?=public_age_badge($info['age_rating'])?></button><p><?=e($info['genre'])?> · <?=(int)$info['duration_minutes']?> min</p><p class="synopsis"><?=e($info['synopsis'])?></p></div><div class="session-groups"><?php foreach($movie['sessions'] as $session):$available=max(0,(int)$session['capacity']-(int)$session['sold']-(int)$session['held']);$canBuy=$onlineSalesEnabled&&$available>0;?><a class="session-button <?=$canBuy?'':'sold-out'?> <?=!$onlineSalesEnabled?'sales-disabled':''?>" <?=$canBuy?'href="/?action=seats&amp;showtime_id='.(int)$session['id'].'"':'aria-disabled="true"'?>><strong><?=e(date('H:i',strtotime($session['starts_at'])))?></strong><span><?=e(ucfirst($session['audio_type']))?></span><span class="format-badges"><?=public_session_format_badges($session)?></span><small><?=!$onlineSalesEnabled?'Venda online pausada':($available>0?$available.' lugares':'Esgotada')?></small><?php if($available>0):?><em>Comprar</em><?php endif;?></a><?php endforeach;?></div></div></article><?=public_movie_modal($info,$movie['sessions'])?><?php endforeach;?><?php if(!$movies):?><section class="empty-state"><strong>Nenhuma sessão nesta data</strong><p>Escolha outro dia na régua acima.</p></section><?php endif;?></div>
    <?php }
});
