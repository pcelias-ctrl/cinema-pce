...
function public_layout(string $title, array $cinema, ?array $customer, string $action, string $message, string $error, callable $content): void
{
    ...
    <footer class="portal-footer"><div><strong><?=e($cinema['cinema_name'])?></strong><span><?=e($cinema['address'])?></span><span class="developer-credit">Desenvolvido por Paulo Elias. <a href="tel:+5519981498510">(19) 98149-8510</a>.</span></div><nav><a href="/?action=privacy">Privacidade</a><a href="/?action=cookies">Cookies</a></nav></footer>
...
public_layout($action === 'catalog' ? 'Ingressos' : 'Minha conta', $cinema, $customer, $action, $message, $error, function () use ($action,$cinema,$customer,$dates,$selectedDate,$parsedDate,$movies,$seatContext,$orderContext,$paymentContext,$accountOrders,$products,$portalSettings) {
    ...
    <?php } else { ?>
        ...
        <div class="movie-program"><?php foreach($movies as $movie):$info=$movie['info'];?><article class="program-card"><div class="poster"><?php if($info['has_cover']):?><img src="/?action=cover&id=<?=(int)$info['movie_id']?>" alt="Capa de <?=e($info['title'])?>"><?php else:?><div class="poster-placeholder">SEM CAPA</div><?php endif;?></div><div class="program-info"><div><div class="movie-title-line"><h2><?=e($info['title'])?></h2><?=public_age_badge($info['age_rating'])?></div><p><?=e($info['genre'])?> · <?=(int)$info['duration_minutes']?> min · Classificação <?=e(public_age_rating_label($info['age_rating']))?></p><p class="synopsis"><?=e($info['synopsis'])?></p></div><div class="session-groups"><?php foreach($movie['sessions'] as $session):$available=max(0,(int)$session['capacity']-(int)$session['sold']-(int)$session['held']);?><a class="session-button <?=$available<1?'sold-out':''?>" href="<?=$available>0?'/?action=seats&showtime_id='.(int)$session['id']:'#'?>"><strong><?=e(date('H:i',strtotime($session['starts_at'])))?></strong><span><?=e(ucfirst($session['audio_type']))?> · <?=$session['is_3d']?'3D':'2D'?></span><small><?=$available>0?$available.' lugares':'Esgotada'?></small></a><?php endforeach;?></div></div></article><?php endforeach;?><?php if(!$movies):?><section class="empty-state"><strong>Nenhuma sessão nesta data</strong><p>Escolha outro dia na régua acima.</p></section><?php endif;?></div>
    <?php }
});
