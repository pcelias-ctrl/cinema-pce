...
public_layout($action === 'catalog' ? 'Ingressos' : 'Minha conta', $cinema, $customer, $action, $message, $error, function () use ($action,$cinema,$customer,$dates,$selectedDate,$parsedDate,$movies,$seatContext,$orderContext,$paymentContext,$accountOrders,$products,$portalSettings) {
    ...
    <?php } else { ?>
        ...
        <div class="movie-program"><?php foreach($movies as $movie):$info=$movie['info'];$firstAvailable=null;foreach($movie['sessions'] as $candidate){$candidateAvailable=max(0,(int)$candidate['capacity']-(int)$candidate['sold']-(int)$candidate['held']);if($candidateAvailable>0){$firstAvailable=$candidate;break;}}$buyUrl=$firstAvailable?'/?action=seats&showtime_id='.(int)$firstAvailable['id']:null;?><article class="program-card" <?=$buyUrl?'data-buy-url="'.e($buyUrl).'"':''?>><?php if($buyUrl):?><a class="poster poster-link" href="<?=e($buyUrl)?>" aria-label="Comprar ingresso para <?=e($info['title'])?>"><?php else:?><div class="poster"><?php endif;?><?php if($info['has_cover']):?><img src="/?action=cover&id=<?=(int)$info['movie_id']?>" alt="Capa de <?=e($info['title'])?>"><?php else:?><div class="poster-placeholder">SEM CAPA</div><?php endif;?><?php if($buyUrl):?><span class="poster-cta">Comprar</span></a><?php else:?></div><?php endif;?><div class="program-info"><div><div class="movie-title-line"><h2><?=e($info['title'])?></h2><?=public_age_badge($info['age_rating'])?></div><p><?=e($info['genre'])?> · <?=(int)$info['duration_minutes']?> min · Classificação <?=e(public_age_rating_label($info['age_rating']))?></p><p class="synopsis"><?=e($info['synopsis'])?></p></div><div class="session-groups"><?php foreach($movie['sessions'] as $session):$available=max(0,(int)$session['capacity']-(int)$session['sold']-(int)$session['held']);?><a class="session-button <?=$available<1?'sold-out':''?>" href="<?=$available>0?'/?action=seats&showtime_id='.(int)$session['id']:'#'?>"><strong><?=e(date('H:i',strtotime($session['starts_at'])))?></strong><span><?=e(ucfirst($session['audio_type']))?> · <?=$session['is_3d']?'3D':'2D'?></span><small><?=$available>0?$available.' lugares':'Esgotada'?></small><?php if($available>0):?><em>Comprar</em><?php endif;?></a><?php endforeach;?></div></div></article><?php endforeach;?><?php if(!$movies):?><section class="empty-state"><strong>Nenhuma sessão nesta data</strong><p>Escolha outro dia na régua acima.</p></section><?php endif;?></div>
    <?php }
});
