...
if ($action === 'infinitepay_webhook') {
    ...
    try{
        $verified=InfinitePay::verifyPayment($portalSettings,$payload);
        $expectedAmount=(int)round((float)$local['total_amount']*100);$paidAmount=(int)($verified['paid_amount']??0);
        if(!empty($verified['paid'])&&$paidAmount===$expectedAmount){$method=($verified['capture_method']??$payload['capture_method']??'pix')==='credit_card'?'cartao':'pix';$db->prepare('UPDATE public_orders SET payment_method=?,provider_transaction_nsu=?,provider_payload=? WHERE id=?')->execute([$method,(string)($payload['transaction_nsu']??''),json_encode($verified,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$orderId]);PublicPortal::finalizePaidOrder($db,$orderId,$verified);}
...
if ($action === 'pay' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try{
        ...
        $requestedGateway=($_POST['payment_gateway']??$portalSettings['payment_gateway'])==='infinitepay'?'infinitepay':'pagarme';
        ...
        $total=(float)$order['tickets_total']+$productTotal;$db->prepare("UPDATE public_orders SET payment_method=?,payment_gateway=?,products_total=?,total_amount=?,status='aguardando_pagamento' WHERE id=?")->execute([$method,$gateway,$productTotal,$total,(int)$order['id']]);
        ...
        $provider=Pagarme::createOrder($portalSettings,$order,$customer,$items,$method,trim($_POST['card_token']??'')?:null);$charge=$provider['charges'][0]??[];$transaction=$charge['last_transaction']??[];
...
public_layout($action === 'catalog' ? 'Ingressos' : 'Minha conta', $cinema, $customer, $action, $message, $error, function () use ($action,$cinema,$customer,$dates,$selectedDate,$parsedDate,$movies,$seatContext,$orderContext,$paymentContext,$accountOrders,$products,$portalSettings) {
    ...
    <?php } elseif ($action === 'checkout') { if(!$orderContext){?><section class="empty-state"><strong>Reserva indisponível</strong><p>Escolha as poltronas novamente.</p><a class="portal-button primary" href="/">Ver sessões</a></section><?php return;}?>
        ...
        <?php $pagarmeReady=!empty($portalSettings['pagarme_public_key'])&&!empty($portalSettings['pagarme_secret_encrypted']);$infiniteReady=!empty($portalSettings['infinitepay_handle']);$gateway=$portalSettings['payment_gateway']==='infinitepay'&&$infiniteReady?'infinitepay':($pagarmeReady?'pagarme':'infinitepay');$gatewayReady=$pagarmeReady||$infiniteReady;?>
        <form method="post" action="/?action=pay" class="public-checkout" id="public-checkout-form" data-gateway="<?=e($gateway)?>" data-ticket-total="<?=e($orderContext['tickets_total'])?>" data-pagarme-key="<?=e($portalSettings['pagarme_public_key'])?>">
            ...
            <aside class="checkout-summary"><h2>Resumo</h2><div><span>Ingressos</span><strong>R$ <?=e(number_format((float)$orderContext['tickets_total'],2,',','.'))?></strong></div><div><span>Produtos</span><strong id="checkout-products-total">R$ 0,00</strong></div><div class="grand"><span>Total</span><strong id="checkout-grand-total">R$ <?=e(number_format((float)$orderContext['total_amount'],2,',','.'))?></strong></div><label>Gateway<select name="payment_gateway" id="public-payment-gateway"><option value="pagarme" <?=$gateway==='pagarme'?'selected':''?> <?=$pagarmeReady?'':'disabled'?>>Pagar.me</option><option value="infinitepay" <?=$gateway==='infinitepay'?'selected':''?> <?=$infiniteReady?'':'disabled'?>>InfinitePay</option></select></label><div id="pagarme-payment-options"><label>Pagamento<select name="payment_method" id="public-payment-method"><option value="pix">Pix</option><option value="cartao">Cartão</option></select></label><div class="card-fields" id="public-card-fields" hidden><label>Número do cartão<input id="card-number" inputmode="numeric" autocomplete="cc-number"></label><label>Nome impresso<input id="card-holder" autocomplete="cc-name"></label><div><label>Validade<input id="card-expiry" placeholder="MM/AA" inputmode="numeric" autocomplete="cc-exp"></label><label>CVV<input id="card-cvv" inputmode="numeric" maxlength="4" autocomplete="cc-csc"></label></div></div></div><div class="gateway-choice" id="infinitepay-payment-options"><strong>InfinitePay</strong><span>Escolha Pix ou cartão na próxima etapa.</span></div><button class="portal-button primary" id="public-pay-button" <?=(!(int)$portalSettings['sales_enabled']||!$gatewayReady)?'disabled':''?>>Continuar para pagamento</button><?php if(!(int)$portalSettings['sales_enabled']):?><small>Pagamento online temporariamente indisponível.</small><?php elseif(!$gatewayReady):?><small>Gateway aguardando configuração.</small><?php else:?><small>Escolha o gateway e prossiga com segurança.</small><?php endif;?></aside>
        </form>
