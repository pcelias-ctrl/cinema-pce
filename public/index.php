...
if ($action === 'pay' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try{
        ...
        $total=(float)$order['tickets_total']+$productTotal;$db->prepare("UPDATE public_orders SET payment_method=?,payment_gateway=?,products_total=?,total_amount=?,status='aguardando_pagamento',expires_at=DATE_ADD(NOW(),INTERVAL 30 MINUTE) WHERE id=?")->execute([$method,$gateway,$productTotal,$total,(int)$order['id']]);
        $db->prepare('UPDATE public_seat_holds SET expires_at=DATE_ADD(NOW(),INTERVAL 30 MINUTE) WHERE order_id=?')->execute([(int)$order['id']]);
        ...
        $order['total_amount']=$total;$order['products_total']=$productTotal;$order['payment_method']=$method;$order['expires_epoch']=time()+1800;$db->commit();
