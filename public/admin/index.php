...
function app_url(string $route, array $params = []): string
{
    ...
    return $scheme . '://' . $host . '/admin/index.php?' . $query;
}
...
function ensure_product_tables(): void
{
    ...
    db()->exec("CREATE TABLE IF NOT EXISTS products (
        ...
        sku VARCHAR(60) NULL UNIQUE, price DECIMAL(10,2) NOT NULL, stock_quantity INT NULL, sort_order SMALLINT NOT NULL DEFAULT 0,
    ...
    $sortColumn = db()->query("SHOW COLUMNS FROM products LIKE 'sort_order'")->fetch();
    if (!$sortColumn) {
        db()->exec('ALTER TABLE products ADD COLUMN sort_order SMALLINT NOT NULL DEFAULT 0 AFTER stock_quantity');
    }
...
try {
    ...
    if ($route === 'public_settings') {
        Auth::requireAdmin();
        $settings = PublicPortal::settings(db());
        $saved = false;
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            verify_csrf();
            $pagarmeSecret = trim($_POST['pagarme_secret'] ?? '');
            $webhookPassword = trim($_POST['pagarme_webhook_password'] ?? '');
            $googleSecret = trim($_POST['google_client_secret'] ?? '');
            $pagarmeEncrypted = $pagarmeSecret !== '' ? SettingCrypto::encrypt($pagarmeSecret) : $settings['pagarme_secret_encrypted'];
        ...
        $webhookUrl = rtrim((string) config_value('app_url'), '/') . '/?action=pagarme_webhook';
        $infiniteWebhookUrl = rtrim((string) config_value('app_url'), '/') . '/?action=infinitepay_webhook';
        if ($settings['infinitepay_handle']) $infiniteWebhookUrl .= '&token=' . InfinitePay::webhookToken((string) $settings['infinitepay_handle']);
        layout('Venda Online', function() use($settings,$saved,$webhookUrl,$infiniteWebhookUrl){?>
            ...
            <form method="post" enctype="multipart/form-data" class="form wide"><input type="hidden" name="csrf_token" value="<?=e(csrf_token())?>">
                <section class="panel"><div class="settings-title"><h2>Portal público</h2><label class="check-label"><input type="checkbox" name="sales_enabled" value="1" <?=$settings['sales_enabled']?'checked':''?>> Habilitar venda de ingressos na internet</label></div><p class="muted">Quando desabilitada, a programação continua visível, mas a escolha de poltronas, reservas e pagamentos ficam bloqueados.</p><div class="columns"><label>Processamento dos pagamentos<select name="payment_gateway"><option value="mixed" <?=$settings['payment_gateway']==='mixed'?'selected':''?>>Mesclar: Pix InfinitePay + cartão Pagar.me</option><option value="pagarme" <?=$settings['payment_gateway']==='pagarme'?'selected':''?>>Somente Pagar.me</option><option value="infinitepay" <?=$settings['payment_gateway']==='infinitepay'?'selected':''?>>Somente InfinitePay</option></select></label><label>Reserva das poltronas (minutos)<input name="hold_minutes" type="number" min="5" max="30" value="<?=e($settings['hold_minutes'])?>"></label><label>Bloquear venda pública antes da sessão (minutos)<input name="public_sale_cutoff_minutes" type="number" min="0" max="240" value="<?=e($settings['public_sale_cutoff_minutes'] ?? 45)?>"><small>Exemplo: 45 bloqueia compras online faltando 45 minutos.</small></label><label>E-mail para privacidade<input name="privacy_contact_email" type="email" value="<?=e($settings['privacy_contact_email'])?>"></label><label>Versão da política de cookies<input name="cookie_policy_version" value="<?=e($settings['cookie_policy_version'])?>"></label></div></section>
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                if ($action === 'toggle') {
                    ...
                    if ($id > 0) {
                        $sql = 'UPDATE products SET category_id=?, name=?, sku=?, price=?, stock_quantity=?, sort_order=?';
                        $params = [$categoryId, $name, $sku, $price, $stock, $sortOrder];
                        if ($image) {
                        ...
                        db()->prepare('INSERT INTO products(category_id,name,sku,price,stock_quantity,sort_order,image_mime,image_data) VALUES(?,?,?,?,?,?,?,?)')->execute([$categoryId, $name, $sku, $price, $stock, $sortOrder, $image['mime'] ?? null, $image['data'] ?? null]);
                    }
                }
