...
try {
    ...
    if ($route === 'public_settings') {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $webhookSecret = trim($_POST['pagarme_webhook_secret'] ?? '');
            ...
            $webhookEncrypted = $webhookSecret !== '' ? SettingCrypto::encrypt($webhookSecret) : $settings['pagarme_webhook_secret_encrypted'];
            $googleEncrypted = $googleSecret !== '' ? SettingCrypto::encrypt($googleSecret) : $settings['google_client_secret_encrypted'];
            $stmt = db()->prepare('UPDATE public_portal_settings SET sales_enabled=?,hold_minutes=?,pagarme_public_key=?,pagarme_secret_encrypted=?,pagarme_webhook_secret_encrypted=?,google_client_id=?,google_client_secret_encrypted=?,privacy_contact_email=?,cookie_policy_version=? WHERE id=1');
            $stmt->execute([!empty($_POST['sales_enabled'])?1:0,max(5,min(30,(int)($_POST['hold_minutes']??10))),trim($_POST['pagarme_public_key']??'')?:null,$pagarmeEncrypted?:null,$webhookEncrypted?:null,trim($_POST['google_client_id']??'')?:null,$googleEncrypted?:null,trim($_POST['privacy_contact_email']??'')?:null,trim($_POST['cookie_policy_version']??'1.0')?:'1.0']);
            $settings = PublicPortal::settings(db());
            $saved = true;
        }
        $webhookSecret = SettingCrypto::decrypt((string) $settings['pagarme_webhook_secret_encrypted']);
        $webhookUrl = rtrim((string) config_value('app_url'), '/') . '/?action=pagarme_webhook';
        if ($webhookSecret !== '') {
            $webhookUrl .= '&token=' . hash('sha256', $webhookSecret);
        }
        layout('Venda Online', function() use($settings,$saved,$webhookUrl){?>
            ...
            <form method="post" class="form wide"><input type="hidden" name="csrf_token" value="<?=e(csrf_token())?>">
                <section class="panel"><div class="settings-title"><h2>Portal público</h2><label class="check-label"><input type="checkbox" name="sales_enabled" value="1" <?=$settings['sales_enabled']?'checked':''?>> Ativar pagamentos online</label></div><div class="columns"><label>Reserva das poltronas (minutos)<input name="hold_minutes" type="number" min="5" max="30" value="<?=e($settings['hold_minutes'])?>"></label><label>E-mail para privacidade<input name="privacy_contact_email" type="email" value="<?=e($settings['privacy_contact_email'])?>"></label><label>Versão da política de cookies<input name="cookie_policy_version" value="<?=e($settings['cookie_policy_version'])?>"></label></div></section>
                <section class="panel"><div class="settings-title"><h2>Pagar.me</h2><span class="status-badge <?=($settings['pagarme_public_key']&&$settings['pagarme_secret_encrypted'])?'active':'inactive'?>"><?=($settings['pagarme_public_key']&&$settings['pagarme_secret_encrypted'])?'Configurado':'Pendente'?></span></div><div class="columns"><label>Chave pública<input name="pagarme_public_key" value="<?=e($settings['pagarme_public_key'])?>" autocomplete="off"></label><label>Chave secreta<input name="pagarme_secret" type="password" autocomplete="new-password" placeholder="<?=$settings['pagarme_secret_encrypted']?'Configurada; deixe vazio para manter':''?>"></label><label>Segredo do webhook<input name="pagarme_webhook_secret" type="password" autocomplete="new-password" placeholder="<?=$settings['pagarme_webhook_secret_encrypted']?'Configurado; deixe vazio para manter':''?>"></label></div><p class="muted"><strong>Webhook:</strong> <?=e($webhookUrl)?></p></section>
