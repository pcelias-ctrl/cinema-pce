...
try {
    ...
    if ($route === 'public_settings') {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $gateway = ($_POST['payment_gateway'] ?? 'pagarme') === 'infinitepay' ? 'infinitepay' : 'pagarme';
        ...
        layout('Venda Online', function() use($settings,$saved,$webhookUrl,$infiniteWebhookUrl){?>
            ...
            <form method="post" class="form wide"><input type="hidden" name="csrf_token" value="<?=e(csrf_token())?>">
                <section class="panel"><div class="settings-title"><h2>Portal público</h2><label class="check-label"><input type="checkbox" name="sales_enabled" value="1" <?=$settings['sales_enabled']?'checked':''?>> Ativar pagamentos online</label></div><div class="columns"><label>Gateway de pagamento<select name="payment_gateway"><option value="pagarme" <?=$settings['payment_gateway']==='pagarme'?'selected':''?>>Pagar.me</option><option value="infinitepay" <?=$settings['payment_gateway']==='infinitepay'?'selected':''?>>InfinitePay</option></select></label><label>Reserva das poltronas (minutos)<input name="hold_minutes" type="number" min="5" max="30" value="<?=e($settings['hold_minutes'])?>"></label><label>E-mail para privacidade<input name="privacy_contact_email" type="email" value="<?=e($settings['privacy_contact_email'])?>"></label><label>Versão da política de cookies<input name="cookie_policy_version" value="<?=e($settings['cookie_policy_version'])?>"></label></div></section>
