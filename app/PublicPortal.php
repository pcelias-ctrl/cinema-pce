    ...
    public static function finalizePaidOrder(PDO $db, int $orderId, array $providerPayload): void
    {
        ...
        try {
            ...
            try {
                ...
                $detailsStmt=$db->prepare('SELECT showtimes.starts_at,showtimes.audio_type,showtimes.is_3d,movies.title,rooms.name room_name FROM showtimes INNER JOIN movies ON movies.id=showtimes.movie_id INNER JOIN rooms ON rooms.id=showtimes.room_id WHERE showtimes.id=?');$detailsStmt->execute([(int)$order['showtime_id']]);$details=$detailsStmt->fetch()?:[];
                ...
                $schedule=!empty($details['starts_at'])?date('d/m/Y H:i',strtotime((string)$details['starts_at'])):'';$format=!empty($details['is_3d'])?'3D':'2D';$audio=ucfirst((string)($details['audio_type']??''));
