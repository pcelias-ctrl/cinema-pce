...
final class PublicPdf
{
    public static function tickets(PDO $db, string $orderCode, int $customerId): string
    {
        ...
        foreach($rows as $index=>$row){$html.='<section class="ticket '.($index?'page':'').'">'.self::logo($cinema).'<div class="kind">INGRESSO</div><h1>'.self::e($row['title']).'</h1><div class="grid"><p><span>Data</span><strong>'.date('d/m/Y H:i',strtotime($row['starts_at'])).'</strong></p><p><span>Sala</span><strong>'.self::e($row['room_name']).'</strong></p><p><span>Poltrona</span><strong class="seat">'.self::e($row['seat_code']).'</strong></p><p><span>Tipo</span><strong>'.self::e(ucfirst($row['ticket_type'])).'</strong></p><p><span>Áudio</span><strong>'.self::e(ucfirst($row['audio_type'])).'</strong></p><p><span>Exibição</span><strong>'.($row['is_3d']?'3D':'2D').'</strong></p></div><div class="qr"><img src="'.self::qr((string)$row['qr_token']).'"><small>'.self::e($row['qr_token']).'</small></div><div class="code">Pedido '.self::e($row['sale_code']).'</div></section>';}
